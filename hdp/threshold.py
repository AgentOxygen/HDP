...
from hdp.utils import get_time_stamp, get_version
...
def compute_threshold(baseline_data: xarray.DataArray, percentiles: np.ndarray, no_season: bool=False, rolling_window_size: int=7, fixed_value: float=None):
    ...
    history_str = f"({get_time_stamp()}) Threshold data computed by HDP v{get_version('hdp_python')}.\n"
    ...
    ds = xarray.Dataset(
        ...
        attrs=dict(
            description=f"Extreme heat threshold dataset generated by Heatwave Diagnostics Package (HDP v{get_version('hdp_python')})",
            hdp_version=get_version('hdp_python'),
        )
    )
