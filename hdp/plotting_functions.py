...
def create_single_hw_plot(hw_ds):
	...
	if np.min(cyclic_values) < 0:
		cmap="bwr"
	else:
		cmap="Reds"
	ax3_contour = ax3.contourf(cyclic_lons, hw_ds.lat.values, cyclic_values, transform=ccrs.PlateCarree(), cmap=cmap)
	...
	if np.min(cyclic_values) < 0:
		...
		cmap="Reds"
	ax4_contour = ax4.contourf(cyclic_lons, hw_ds.lat.values, cyclic_values, transform=ccrs.PlateCarree(), cmap=cmap)
...
def create_multi_threshold_plot(hw_ds):
	...
	if np.min(cyclic_values) < 0:
		...
		cmap="Reds"
	ax2_contour = ax2.contourf(cyclic_lons, hw_ds.lat.values, cyclic_values, transform=ccrs.PlateCarree(), cmap=cmap)
...
def create_threshold_plot(hw_ds):
	if "percentile" in hw_ds and hw_ds.percentile.size > 1:
