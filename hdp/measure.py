...
import numba as nb
import xarray
from hdp.utils import get_time_stamp, add_history, getVersion
...
def apply_heat_index(temp: xarray.DataArray, rh: xarray.DataArray) -> xarray.DataArray:
    ...
    hi_da = add_history(hi_da, f"({get_time_stamp()}) Converted to heat index using '{rh.name}' relative humidity, renamed from '{temp.name}' to '{hi_da.name}'.\n")
    return hi_da
...
def format_standard_measures(temp_datasets: list[xarray.DataArray], rh: xarray.DataArray = None):
    ...
    agg_ds.attrs = {
        "description": f"Heat measurement dataset generated by Heatwave Diagnostics Package (HDP v{getVersion()})",
        "hdp_version": getVersion(),
    }
